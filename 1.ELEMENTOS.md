...
- **De sistema** (Sistema operativo, drivers -controladores de dispositivos-)
- **De aplicación** (Suite ofimática, Navegador, Edición de imagen, ...)
- **De desarrollo** (Editores, compiladores, interpretes, ...)

[//]: # (gfgsfdg esto es un comentario)
...
El disco duro se considera un periférico de entrada y salida (E/S).
LA CPU se llama tambieén UCP (Unidad Central de Procesos), procesador o microprocesador (micro también).
...
Predictivos: Estos modelos clásicos planifican todo hasta el último detalle. Tienen un planificación muy rígida por lo que no se suelen usar.
Modelos evoluivos o incrementales: No están definidos al 100%. Van cambiando con el tiempo.
...
Se llama modelo en cascada porque se pasa de una fase a la siguiente pero nunca volviendo a la anterior, por lo que es un modelo muy rígido.

- Modelo de mayor antigüedad.
- Identifica muy bien las fases principales del desarrollo software.
- Las fases han de realizarse en el orden indicado. No es flexible.
...
Es parecido al anterior pero en vez de presentarlo en cascada se representa en V.
Lo importante de este modelo es que al principio se acuerdan con el cliente unos criterios de validación y al final del proceso deben 

Validar: Es que se está haciendo el producto correcto que pide el cliente. Se fijan unos criterios. Cuando se termina el proceso se tienen que validar esos criterios con el cliente y él tiene que dar el visto bueno. 
Esto seria "Se está haciendo el producto CORRECTO(El que pide el cliente)"
Verificar:  Es que se est´a haciendo el producto se la forma correcta, siguiendo el diseño que hemos hecho.
Esto sería "Se está haciendo el producto CORRECTAMENTE(Según el diseño previo)"
...
- La compilación/interpretación del código fuente se lleva a cabo en dos fases:
  1. __Análisis léxico__ (son como las faltas de ortografía)
  2. __Análisis sintáctico__ (son como errores gramaticales, que las palabras no tenga el orden correcto)
...
- Según la forma en la que operan:
  ...
  - __Imperativos__: indicamos los pasos a seguir para obtener un resultado (la mayoría de lenguajes de programación).
